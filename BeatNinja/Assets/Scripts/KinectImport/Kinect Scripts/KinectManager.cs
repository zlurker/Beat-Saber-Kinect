...
public class KinectManager : MonoBehaviour {
    ...
    protected virtual void RemoveUser(Int64 userId) {
        ...
        //bodyParts[uidIndex].parent.gameObject.SetActive(false);
    ...
    private void DrawSkeleton(Texture2D aTexture, ref KinectInterop.BodyData bodyData, int userId) {
        ...
        /*if(!bodyParts[userId].calibrated) {
            bodyParts[userId].calibrated = true;
            bodyParts[userId].calibrationData = bodyData.joint[1].kinectPos;
            bodyParts[userId].calibrationData.y = 0;
            bodyParts[userId].rotationData = bodyData.headOrientation.eulerAngles;

            bodyParts[userId].parent.eulerAngles = bodyParts[userId].rotationData;
            bodyParts[userId].parent.position = bodyParts[userId].calibrationData;
        }

        Vector3 finalVector = bodyData.joint[7].kinectPos - bodyData.joint[1].kinectPos;
        finalVector.x *= -1;
        //bodyParts[userId].legs[0].localPosition = finalVector;
        bodyParts[userId].legs[0].eulerAngles = bodyData.joint[21].normalRotation.eulerAngles;
        //legs[0].position *= 7.5f;'
        //KinectInput.instance.DrawCollider(bodyData.joint[i].kinectPos,0.05f);

        //KinectInterop.DrawLine(aTexture, (int)posParent.x, (int)posParent.y, (int)posJoint.x, (int)posJoint.y, Color.yellow);
        finalVector = bodyData.joint[11].kinectPos - bodyData.joint[1].kinectPos;
        finalVector.x *= -1;
        //bodyParts[userId].legs[1].localPosition = finalVector;
        bodyParts[userId].legs[1].eulerAngles = bodyData.joint[23].normalRotation.eulerAngles;*/
